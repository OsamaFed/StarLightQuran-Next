...
const getRandomVerse = async (): Promise<VerseData> => {
  try {
    ...
    const response = await fetch(`https://api.alquran.cloud/v1/ayah/${randomVerseNumber}/quran-simple`);
...
const getTodaysVerse = async (): Promise<VerseData> => {
  try {
    ...
    const response = await fetch(`https://api.alquran.cloud/v1/ayah/${verseNumber}/quran-simple`);
    const data = await response.json();
    if (data.code === 200) return data.data;
    throw new Error("Failed to fetch verse");
  } catch (error) {
